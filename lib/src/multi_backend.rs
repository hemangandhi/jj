...
impl MultiBackend {
            ...
            if let TreeValue::Tree(inner_id) = tree_entry.value() {
                tree_proto.inner_trees.push(
                    crate::protos::multi_backend_tree_ids::InnerBackendTreeId {
                        ...
                        is_tree_multi_backend: self
                            .get_backend_at_path(path.join(tree_entry.name()).as_ref())
                            .is_none(),
                    },
                )
            } else {
    ...
    fn write_root_tree(&self) -> Result<(), BackendInitError> {
        let root_tree = MultiBackendTreeIds {
            id: [0; HASH_LENGTH].to_vec(),
            inner_trees: vec![],
            root_path: RepoPath::root().as_internal_file_string().to_string(),
        };
        let (table, _lock) = self
            .trees
            .get_head_locked()
            .map_err(|e| BackendInitError(Box::new(e)))?;
        let mut mut_table = table.start_mutation();
        mut_table.add_entry([0; HASH_LENGTH].to_vec(), root_tree.encode_to_vec());
        self.trees
            .save_table(mut_table)
            .map_err(|e| BackendInitError(Box::new(e)))?;
        Ok(())
    ...
    fn get_trees_inside_backends<'a>(
        ...
        let mut merge_conflict_per_path: HashMap<RepoPathBuf, Vec<(TreeId, String)>> =
            HashMap::new();
        let mut trees_to_traverse_under: HashMap<TreeId, String> = HashMap::new();
        ...
        for (merge_id, label) in merges_and_labels {
            ...
            for sub_tree in tree.inner_trees {
                ...
                if !sub_tree.is_tree_multi_backend {
                    ...
                    let map_entry = trees_to_traverse_under.entry(sub_tree_id.clone());
                    if let Entry::Occupied(_occ) = map_entry {
                        return Err(BackendError::Other(Box::new(MultiBackendError(format!(
                            "Colliding tree ids! {} duplicated, duplicate is at {:?}",
        ...
        while !trees_to_traverse_under.is_empty() {
            let mut next_trees_to_traverse_under: HashMap<TreeId, String> = HashMap::new();
            for (tree_id, label) in trees_to_traverse_under.drain() {
                let tree = self
                ...
                for sub_tree in tree.inner_trees {
                    ...
                    if !sub_tree.is_tree_multi_backend {
                        ...
                        let map_entry = next_trees_to_traverse_under.entry(sub_tree_id.clone());
                        if let Entry::Occupied(_occ) = map_entry {
                            return Err(BackendError::Other(Box::new(MultiBackendError(format!(
                                "Colliding tree ids! {} duplicated, duplicate is at {:?}",
                                sub_tree_id.to_string(),
...
impl Backend for MultiBackend {
    ...
    async fn read_tree(&self, path: &RepoPath, id: &TreeId) -> BackendResult<Tree> {
        if let Some((backend, inner_path)) = self.get_backend_at_path(path) {
            return backend.read_tree(inner_path, id).await;
        }
