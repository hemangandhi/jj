...
impl<'a> BackendAndCommits<'a> {
    ...
    fn get_copy_records(
        ...
        self.backend
            .get_copy_records(self.paths.as_deref(), &self.root, &self.head)
            .map(move |stream: BoxStream<'_, BackendResult<CopyRecord>>| {
                stream.map(move |maybe_copy: BackendResult<CopyRecord>| {
...
impl MultiBackend {
    ...
    fn traverse_backends_for_empty_tree(
        ...
        let mut layer_entries = layer
            .children()
            .map(|(path_bit, sub_tree)| {
                let owned_bit = path_bit.to_owned();
                let inner_value = self.traverse_backends_for_empty_tree(
                    &owned_bit,
                    &path.join(path_bit),
                    sub_tree,
                )?;
        ...
        if let Some(inner_backend) = layer.value() {
            layer_entries.push((
                parent_bit.clone(),
                TreeValue::Tree(inner_backend.empty_tree_id().clone()),
            ));
        }
        ...
        let tree_id = self
            .write_multi_backend_tree(path.parent().unwrap_or(&RepoPath::root()), &tree)
            .map_err(|e| BackendInitError(Box::new(e)))?;
        Ok(TreeValue::Tree(tree_id))
    }

    fn write_root_tree(&mut self) -> Result<(), BackendInitError> {
        let mut top_level_children = self
            .backends
            .children()
            .map(|(bit, sub_tree)| {
                let owned_bit = bit.to_owned();
                let inner_tree =
                    self.traverse_backends_for_empty_tree(&owned_bit, &RepoPath::root(), sub_tree)?;
        ...
        self.root_tree_id = self
            .write_multi_backend_tree(&RepoPath::root(), &tree)
            .map_err(|e| BackendInitError(Box::new(e)))?;
        Ok(())
    }
    ...
    pub fn init_with_backends_at_paths(
        ...
        for (i, inner_repo_path) in repo_paths.iter().enumerate() {
            ...
            std::fs::write(inner_store_dir.join("type"), inner_backend.name())
                .map_err(|e| BackendInitError(Box::new(e)))?;
            // TODO: consider what actual path this should be and how we should
            // deal with it. Perhaps pointing to the .git is for the best here,
            // but perhaps it's a bad idea for the CLI.
            let inner_repo_parent = inner_repo_path
                .parent()
                .expect("Repo path should end in .git subdir?");
            tree.add(inner_repo_parent).set_value(Some(inner_backend));
            path_map.insert(
                inner_repo_parent.as_internal_file_string().to_string(),
    ...
    fn get_trees_inside_backends<'a>(
        ...
        for (merge_id, label) in merges_and_labels {
            ...
            for sub_tree in tree.inner_trees {
                ...
                if !sub_tree.is_tree_multi_backend {
                    ...
                    let map_entry = trees_to_traverse_under.entry((
                        sub_tree_id.clone(),
                        total_path.as_internal_file_string().to_owned(),
                    ));
        ...
        while !trees_to_traverse_under.is_empty() {
            let mut next_trees_to_traverse_under: HashMap<(TreeId, String), String> =
                HashMap::new();
            for ((tree_id, _path), label) in trees_to_traverse_under.drain() {
                ...
                for sub_tree in tree.inner_trees {
                    ...
                    if !sub_tree.is_tree_multi_backend {
                        ...
                        let map_entry = next_trees_to_traverse_under.entry((
                            sub_tree_id.clone(),
                            total_path.as_internal_file_string().to_owned(),
                        ));
                        if let Entry::Occupied(_occ) = map_entry {
...
impl Backend for MultiBackend {
    ...
    async fn read_tree(&self, path: &RepoPath, id: &TreeId) -> BackendResult<Tree> {
        ...
        if !self.is_tree_at_path_multi_backend(path, id) {
            ...
            if let Some((backend, inner_path)) = self.get_backend_at_path(path) {
                return backend.read_tree(inner_path, id).await.map_err(|e| {
                    BackendError::ReadObject {
                        object_type: id.object_type(),
                        hash: id.to_string(),
                        source: Box::new(e),
    ...
    async fn write_commit(
        ...
        for (path, backend, commit) in self.split_commit_per_backend(&contents)? {
            let inner_commit_id = backend
                .write_commit(commit, sign_with.as_mut().map(|x| &mut **x))
                .await?
                .0;
    ...
    fn get_copy_records(
        ...
        for inner_commit in head_proto.inner_backend_commits {
            ...
            if let Entry::Occupied(mut backend_and_commits) = map_entry {
                backend_and_commits.get_mut().head =
                    CommitId::from_bytes(inner_commit.commit_id.as_ref());
                continue;
            }
            let backend = self
                .get_backend_at_path(path)
                ...
                .0;
            map_entry.or_insert(BackendAndCommits::from_head_commit(
                backend,
                &inner_commit.commit_id,
                paths,
                path,
            ));
        }

        let future_copies = path_backend_commits_map
            .drain()
            .map(
                move |(path, backend_info): (RepoPathBuf, BackendAndCommits<'_>)| {
